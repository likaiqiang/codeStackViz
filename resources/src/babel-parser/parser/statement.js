import { tokenIsIdentifier, tokenIsKeywordOrIdentifier, tokenIsLoop, tokenIsTemplate, tt, getExportedToken } from "../tokenizer/types.js";
...
import { BIND_CLASS, BIND_LEXICAL, BIND_VAR, BIND_FUNCTION, SCOPE_CLASS, SCOPE_FUNCTION, SCOPE_OTHER, SCOPE_SIMPLE_CATCH, SCOPE_STATIC_BLOCK, SCOPE_SUPER, CLASS_ELEMENT_OTHER, CLASS_ELEMENT_INSTANCE_GETTER, CLASS_ELEMENT_INSTANCE_SETTER, CLASS_ELEMENT_STATIC_GETTER, CLASS_ELEMENT_STATIC_SETTER, BIND_CATCH_PARAM } from "../util/scopeflags.js";
import { ExpressionErrors } from "./util.js";
import { PARAM, functionFlags } from "../util/production-parameter.js";
import { newExpressionScope, newParameterDeclarationScope } from "../util/expression-scope.js";
...
export var ParseStatementFlag = {
  StatementOnly: 0, //只允许解析普通语句
  AllowImportExport: 1, //允许import/export
  AllowDeclaration: 2, //允许声明，包含所有声明
  AllowFunctionDeclaration: 4, //允许函数声明，与AllowDeclaration的区别是：这个只允许函数声明
  AllowLabeledFunction: 8 // 允许带有标签的函数声明，比如async函数
};
...
export default class StatementParser extends ExpressionParser {
  ...
  parseStatementLike(flags) {
    let decorators = null;
    if (this.match(26)) { //匹配JavaScript原生装饰器语法？@log这种的？必要的时候举个例子
  ...
  parseStatementContent(flags, decorators) {
    ...
    switch (starttype) { // 这里需要分别举例子
      ...
      case 75:
        {
          if (!allowDeclaration) { // 这里需要核实为什么要做这个判断
            this.raise(Errors.UnexpectedLexicalDeclaration, {
              at: this.state.startLoc
            });
          }
  ...
  parseVarStatement(node, kind, allowMissingInitializer = false) {
    this.next();
    this.parseVar(node, false, kind, allowMissingInitializer); // 解析变量声明，并转换为ast，参数isFor表示变量声明是否在for循环中
    this.semicolon();
  ...
  parseBlockBody(node, allowDirectives, topLevel, end, afterBlockParse) {
    const body = node.body = []; //body是个数组，需要验证是否一个语句占一个元素
    const directives = node.directives = []; //指令节点,比如"use strict"
  ...
  applyImportPhase(node, isExport, phase, loc) {
    if (isExport) {
      {
        if (phase === "module") {
          throw new Error("Assertion failure: export declarations do not support the 'module' phase.");
        }
      }
      return;
    }
