import { isIdentifierStart, isIdentifierChar, isStrictBindReservedWord, isStrictBindOnlyReservedWord, isKeyword, isReservedWord, isStrictReservedWord } from '@babel/helper-validator-identifier';
...
function ParseErrorEnum(argument, syntaxPlugin) {
  ...
  for (const reasonCode of Object.keys(argument)) {
    ...
    const _ref2 = typeof template === "string" ? {
      ...
      rest = _objectWithoutPropertiesLoose$1(_ref2, _excluded2$1);
...
function tokenKeywordOrIdentifierIsKeyword(token) {
  // 判断一个关键字或者标识符是关键字，并不是标识符
  return token <= 92;
}
function tokenIsKeywordOrIdentifier(token) {
  // 判断一个token是否是关键字或者标识符
...
const BIND_KIND_VALUE = 0b0000000_0000_01,
  BIND_KIND_TYPE = 0b0000000_0000_10,
  BIND_SCOPE_VAR = 0b0000000_0001_00,
  BIND_SCOPE_LEXICAL = 0b0000000_0010_00,
  BIND_SCOPE_FUNCTION = 0b0000000_0100_00,
...
var ParseStatementFlag = {
  ...
  //允许函数声明，与AllowDeclaration的区别是：这个只允许函数声明
...
class StatementParser extends ExpressionParser {
  ...
  parseStatementLike(flags) {
    let decorators = null;
    if (this.match(26)) {
      //匹配JavaScript原生装饰器语法？@log这种的？必要的时候举个例子
